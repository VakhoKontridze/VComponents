...
// MARK: - V Static Pager Tab View (Wrapped Indicator)
/// View that switches between child views, attributed with pager with wrapped rectangular indicator.
///
        ...
        Content: View
{
    ...
    private var tabIndicator: some View {
        ZStack(alignment: .leading, content: {
            ...
            RoundedRectangle(cornerRadius: uiModel.tabIndicatorCornerRadius)
                .frame(width: tabIndicatorWidth)
                .offset(x: tabIndicatorOffset)

                .foregroundColor(uiModel.tabIndicatorColor)

                .animation(enablesTabIndicatorAnimations ? uiModel.tabIndicatorAnimation : nil, value: tabIndicatorWidth)
                .animation(enablesTabIndicatorAnimations ? uiModel.tabIndicatorAnimation : nil, value: tabIndicatorOffset)
        })
        .frame(height: uiModel.tabIndicatorHeight)
    }
        ...
        GeometryReader(content: { tabViewProxy in
            TabView(selection: $selection, content: {
                ForEach(data, id: id, content: { element in
                    content(element)
                        .tag(element)
                        .frame(size: tabViewProxy.size) // Ensures that small content doesn't break page indicator calculation
    ...
    private func calculateIndicatorFrame(
        ...
        if let value: CGFloat = calculateLinearInterpolation(
            ...
            tabIndicatorOffset = value
        }

        if !enablesTabIndicatorAnimations {
            DispatchQueue.main.async(execute: { enablesTabIndicatorAnimations = true })
        }
    }
    ...
    private func calculateLinearInterpolation(
        ...
        for index in 1..<dataSource.count {
            guard contentOffset <= tabContentOffsets[index] else { continue }
